...
class Video:
    ...
    def watch(self):
        if self.current_time < self.duration:
            self.current_time += 1
            print(f"Вы смотрите '{self.title}'. Прошло {self.current_time} секунд(ы).")
        else:
            print(f"Видео '{self.title}' просмотрено полностью.")
        ...
        for video in self.videos:
            if video.title == title:
                ...
                for second in range(video.duration):
                    print(second + 1, end=' ')
                    time.sleep(1)  # Не забудьте импортировать sleep
                print("Конец видео")
                video.current_time = 0  # Сбрасываем текущее время
                return
